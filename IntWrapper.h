...
namespace Detail
{
template<std::size_t size> constexpr void bitTester() {}
...
struct BitInt
{
	...
	static_assert(i / CHAR_BIT > sizeof(std::intmax_t), "Too big!");
};

#define SS(bit)               \
template<> struct BitInt<bit> \
{                             \
	using TI = int##bit##_t;  \
	using TU = uint##bit##_t; \
}
SS(8);
SS(16);
SS(32);
...
template<typename T> inline constexpr std::size_t bitof = sizeof(T) * CHAR_BIT;
} // namespace BitTemplate

inline namespace PtrCast
{
template<typename T>
[[nodiscard]] inline pint ptrToInt(T* ptr) noexcept { return reinterpret_cast<pint>(ptr); }
...
template<typename Fn, std::enable_if_t<Detail::isFunction<Fn>, int> = 0>
...
template<auto f>
[[nodiscard]] auto intToFunc(pint i) noexcept { return intToFunc<decltype(f)>(i); }
} // namespace PtrCast

namespace Limit
{
template<typename T> inline constexpr T max = std::numeric_limits<T>::max();
template<typename T> inline constexpr T min = std::numeric_limits<T>::min();
} // namespace Limit
...
using Port = u16; // TCP/UDP network port

inline namespace Mask
{
template<typename U, std::enable_if_t<std::is_unsigned_v<U>, int> = 0>
union JointInt
{
	using Half = UIntHasBit<bitof<U> / 2>;
